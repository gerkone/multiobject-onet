    ...
    def __init__(
        ...
        if self.cfg["data"]["input_type"] == "pointcloud_crop":
            ...
            if self.cfg["generation"]["sliding_window"]:
                (
                    self.total_input_vol,
                    self.total_query_vol,
                    self.total_reso,
    ...
    def get_vol_info(self, model_path):
        ...
        if self.cfg["data"]["multi_files"] is None:
            ...
            file_path = os.path.join(
                model_path, field_name, "%s_%02d.npz" % (field_name, num)
            )
        ...
        if self.split == "train":
            ...
            lb_input_vol, ub_input_vol = (
                p_c - input_vol_metric / 2,
                p_c + input_vol_metric / 2,
            )
            lb_query_vol, ub_query_vol = (
                p_c - query_vol_metric / 2,
                p_c + query_vol_metric / 2,
            )
