...
class PatchPointCloudField(Field):
    ...
    def __init__(
        self, file_name, transform=None, transform_add_noise=None, multi_files=None
    ):
        self.file_name = file_name
        self.transform = transform
        self.multi_files = multi_files
    ...
    def load(self, model_path, idx, vol):
        ...
        for key in vol["plane_type"]:
            index[key] = coord2index(
                points.copy(), vol["input_vol"], reso=vol["reso"], plane=key
            )
            if key == "grid":
