...
from src.common import map2local, normalize_3d_coordinate, normalize_coordinate
    ...
    def sample_point_feature(self, q, p, fea):
        ...
        if self.sample_mode == "gaussian":
            ...
            dist = -(
                (
                    (
                        p.unsqueeze(1).expand(-1, q.size(1), -1, -1) - q.unsqueeze(2)
                    ).norm(dim=3)
                    + 10e-6
                )
                ** 2
            ...
            weight = 1 / (
                (p.unsqueeze(1).expand(-1, q.size(1), -1, -1) - q.unsqueeze(2)).norm(
                    dim=3
                )
                + 10e-6
            )
