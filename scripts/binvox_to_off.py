...
parser.add_argument('--outputs', '-o', type=str, help='Path to output dir with watertight meshes.')
...
for input_path in tqdm.tqdm(input_paths):
    ...
    mesh = trimesh.voxel.ops.matrix_to_marching_cubes(binvox_data.data, pitch=binvox_data.scale)
    output_path = os.path.join(output_dir, os.path.relpath(input_path, input_dir).replace('.binvox', '.off'))
