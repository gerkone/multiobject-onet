...
def load_meshes(model_list, scale_list, y_angles):
    ...
    for model_idx, model in enumerate(model_list):
        ...
        model_path = os.path.join(
            input_path_watertight, cl, "2_watertight", "%s.off" % m
        )
...
for n_class, n_object in enumerate(n_objects):
    ...
    for split_idx, split_name in enumerate(split_names):
        ...
        while split_item_idx < n_rooms[split_idx]:
            ...
            scales = [1.0, scale_axis] if axis0 else [scale_axis, 1.0]
            ...
            try:
                bboxes = sample_locations(
                    meshes, item_dict["xz_groundplane_range"], item_dict["walls"]
                )
                item_dict["bboxes"] = bboxes
                meshes += get_walls(
                    item_dict["walls"],
                    xz_range=item_dict["xz_groundplane_range"],
                    wall_height=item_dict["wall_height"],
                )

                if use_groundplane:
                    meshes += [
                        get_ground_plane(xz_range=item_dict["xz_groundplane_range"])
                    ]
                ...
                for pcl_idx in range(n_pointcloud_files):
                    ...
                    if pcl_idx == 0:
                        trimesh.Trimesh(
                            vertices=pcl_dict["points"], process=False
                        ).export(os.path.join(folder_path, "pointcloud0.ply"))
