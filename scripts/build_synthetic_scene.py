...
input_path_watertight = '/ssd1/data_shapenet/ShapeNet_watertight'
# folder structure: classes -> idx -> model.off

input_path_splits = '/ssd1/data_shapenet/ShapeNet'
...
for n_class, n_object in enumerate(n_objects):
    print(f"n_class: {n_class}, n_object: {n_object}")
    dataset_item = 0
    ...
    for split_idx, split_name in enumerate(split_names):
        ...
        while split_item_idx < n_rooms[split_idx]:
            ...
            scales = np.insert(scale_axis, 0, 1.) if axis0 else np.insert(scale_axis, len(scale_axis), 1.)
