...
parser = argparse.ArgumentParser(
    "Generate the synthetic room dataset from ShapeNet objects."
)
parser.add_argument(
    "--dataset-size", "-n", type=int, help="Number of rooms to generate.", default=1000
)
parser.add_argument("--input", type=str, help="Path to input dir with voxels.")
parser.add_argument(
    "--input-watertight", type=str, help="Path to input dir with watertight meshes."
)
parser.add_argument(
    "--output", type=str, help="Output path.", default="data/synthetic_scenes"
)
parser.add_argument(
    "--save-scene-files",
    action="store_true",
    help="Save ground truth meshes. Uses disk space.",
)
args = parser.parse_args()

dataset_size = args.dataset_size

# folder structure: classes -> idx -> model.off
input_path_watertight = args.input_watertight
# folder structure: classes -> {train, val, test}.lst
input_path_splits = args.input
output_path = args.output
save_scene_files = args.save_scene_files
# if save_scene_files is True, save GT meshes.
output_path_scene = f"{output_path.split('/')[:-1]}/gt_mesh"
...
split_percentages = [0.75, 0.05, 0.2]
...
xz_groundplane_range = [0.3, 1.0]
y_angle_range = [0.0, 360.0]
...
def load_meshes(model_list, scale_list, y_angles):
    ...
    for model_idx, model in enumerate(model_list):
        cl, m = model.split("/")
        model_path = os.path.join(input_path_watertight, cl, m, "model.off")
        ...
        # Apply y rotation
        r = R.from_euler("y", [y_angles[model_idx]], degrees=True).as_matrix()[0]
        mat = np.eye(4)
        mat[:3, :3] = r
...
def draw_sample(
    ...
    if method == "uniform":
        loc0 = -xz_range / 2.0 + walls[:2] * wall_thickness
        loc_len = (
            xz_range - bounds - walls[:2] * wall_thickness - walls[2:] * wall_thickness
        )
...
def sample_locations(mesh_list, xz_range, walls, max_iter=1000):
    ...
    for mesh_idx, mesh in enumerate(mesh_list):
        ...
        while not found_loc:
            ...
            loc0 = draw_sample(
                bounds, method="uniform", it=mesh_idx, xz_range=xz_range, walls=walls
            )
        ...
        mesh.vertices[:, [0, 2]] = (
            mesh.vertices[:, [0, 2]]
            - np.min(mesh.vertices[:, [0, 2]], axis=0).reshape(1, 2)
            + loc0.reshape(1, 2)
        )
    return bboxes
...
def sample_scales(n_objects, n_class):
    ...
    out_list = [
        scale_interval[0] + np.random.rand() * (scale_interval[1] - scale_interval[0])
...
def sample_pointcloud(
    ...
    walls,
    xz_range,
    ground_plane_only_above=True,
    walls_only_side=True,
):
    ...
    points, normals, semantics, instances = [], [], [], []
    for i, mesh in enumerate(mesh_list):
        ...
        instances_i = np.full_like(pi[:, 0], inst_idx)
        points.append(pi)
    ...
    instances = np.concatenate(instances, axis=0).astype(int)

    # filter for walls and ground plane
    mask = np.ones((points.shape[0])).astype(bool)
    if walls_only_side:
        if walls[0] == 1:
            mask &= points[:, 2] > -xz_range[1] / 2.0 + wall_thickness - 1e-3
        if walls[1] == 1:
            mask &= points[:, 0] > -xz_range[0] / 2.0 + wall_thickness - 1e-3
        if walls[2] == 1:
            mask &= points[:, 2] < +xz_range[1] / 2.0 - wall_thickness + 1e-3
        if walls[3] == 1:
            mask &= points[:, 0] < +xz_range[0] / 2.0 - wall_thickness + 1e-3
...
def sample_iou_points(
    ...
    if scale_to_cube:
        ...
        scale_factor = 1 / (scale_intervals[1] - (-0.5 + groundplane_thickness))
    else:
        scale_factor = 0
    points = (
        np.random.rand(n_iou_poins * n_iou_points_files, 3).astype(np.float32) - 0.5
    ) * (1 + padding)
    ...
    instances = np.ones(n_iou_poins * n_iou_points_files).astype(int) * len(mesh_list)
    for mesh_idx, mesh in enumerate(mesh_list):
        if mesh_idx != len(mesh_list) - 1 and (scale_to_cube or scale_by_constant):
            mesh.vertices[:, 1] = (
                (mesh.vertices[:, 1] + (0.5 - groundplane_thickness)) * scale_factor
            ) - (0.5 + groundplane_thickness)
        occi = check_mesh_contains(mesh, points)
        occ = occ | occi
        if mesh_idx < len(mesh_list) - 1 - np.sum(walls):
            cl_idx = classes.index(cl_list[mesh_idx])
            inst_idx = mesh_idx  # change the semantic label with id
        else:
            cl_idx = -1
            inst_idx = -1
        semantics[occi == 1] = cl_idx
        instances[occi == 1] = inst_idx
    ...
    instances = instances.reshape(n_iou_points_files, n_iou_poins)
...
def get_ground_plane(xz_range=[1.0, 1.0]):
    x_len, z_len = xz_range
    ground_plane = trimesh.creation.box((x_len, groundplane_thickness, z_len))
    bounds = ground_plane.bounds
    ground_plane.vertices = ground_plane.vertices - (bounds.sum(0) / 2).reshape(
        1, 3
    )  # center around origin
    # move ground plane to y = -0.5
    ground_plane.vertices[:, 1] = (
        ground_plane.vertices[:, 1] - min(ground_plane.vertices[:, 1]) - 0.5
    )
    return ground_plane
...
def get_y_angles(n_object):
    angles = y_angle_range[0] + np.random.rand(n_object) * (
        y_angle_range[1] - y_angle_range[0]
    )
    return angles


def get_walls(walls, xz_range=[1.0, 1.0], wall_height=0.2):
    out_list = []
    if walls[0] == 1:
        ...
        wall_x.vertices[:, 1] = (
            wall_x.vertices[:, 1]
            - min(wall_x.vertices[:, 1])
            - 0.5
            + groundplane_thickness
        )
        wall_x.vertices[:, 2] = (
            wall_x.vertices[:, 2] - min(wall_x.vertices[:, 2]) - xz_range[1] / 2.0
        )
    ...
    if walls[1] == 1:
        ...
        wall_x.vertices[:, 0] = (
            wall_x.vertices[:, 0] - min(wall_x.vertices[:, 0]) - xz_range[0] / 2.0
        )
        out_list.append(wall_x)
    if walls[2] == 1:
        wall_x = trimesh.creation.box((xz_range[0], wall_height, wall_thickness))
        ...
        wall_x.vertices[:, 2] = (
            wall_x.vertices[:, 2] - max(wall_x.vertices[:, 2]) + xz_range[1] / 2.0
        )
        out_list.append(wall_x)
    if walls[3] == 1:
        wall_x = trimesh.creation.box((wall_thickness, wall_height, xz_range[1]))
        ...
        wall_x.vertices[:, 0] = (
            wall_x.vertices[:, 0] - max(wall_x.vertices[:, 0]) + xz_range[0] / 2.0
        )
        out_list.append(wall_x)
    return out_list
...
for n_class, n_object in enumerate(n_objects):
    pbar.write(f"n_class: {n_class}, n_object: {n_object}")
    dataset_item = 0
    ...
    for split_idx, split_name in enumerate(split_names):
        ...
        while split_item_idx < n_rooms[split_idx]:
            ...
            scale_axis = (
                np.random.rand(1) * (xz_groundplane_range[1] - xz_groundplane_range[0])
                + xz_groundplane_range[0]
            )
            scales = (
                np.insert(scale_axis, 0, 1.0)
                if axis0
                else np.insert(scale_axis, len(scale_axis), 1.0)
            )
            ...
            item_dict["walls"] = np.zeros((4,)).astype(int)
            item_dict["wall_thickness"] = wall_thickness
            item_dict["wall_height_range"] = wall_height_range
            item_dict["wall_height"] = wall_height_range[0] + np.random.rand(1) * (
                wall_height_range[1] - wall_height_range[0]
            )

            meshes = load_meshes(
                item_dict["objects"], item_dict["scales"], item_dict["y_angles"]
            )
            # try:
            ...
            for pcl_idx in range(n_pointcloud_files):
                pcl_dict = sample_pointcloud(
                    meshes,
                    cl_list,
                    item_dict["walls"],
                    item_dict["xz_groundplane_range"],
                )
                np.savez(
                    os.path.join(pointcloud_dir, "pointcloud_%02d.npz" % pcl_idx),
                    **pcl_dict,
                )
                if pcl_idx == 0:
                    trimesh.Trimesh(vertices=pcl_dict["points"], process=False).export(
                        os.path.join(folder_path, "pointcloud0.ply")
                    )
            ...
            if save_scene_files:
                ...
                merged_mesh.export(
                    os.path.join(output_path_scene_cl, "%08d.obj" % dataset_item)
                )
